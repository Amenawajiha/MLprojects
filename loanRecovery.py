...
df = pd.read_csv("C:\\Users\\shahe\\Downloads\\loan-recovery.csv")

import plotly.graph_objects as go
import plotly.express as px

fig = px.histogram(df, x='Loan_Amount', nbins=30, marginal='violin', opacity=0.7,
                   title='Loan Amount Distribution and Relationship with Monthly Income',
                   labels={'Loan Amount': "Loan Amount(in $)", 'Monthly_Income': "Monthly Income"},
...
sil_score = silhouette_score(df_scaled, df['Borrower_Segment'])
print("Silhouette Score:", sil_score)

# Calculate the Davies-Bouldin index
db_index = davies_bouldin_score(df_scaled, df['Borrower_Segment'])
...
df_test = df_test.merge(df[['Borrower_ID', 'Segment_Name', 'Recovery_Status', 'Collection_Method',
                            'Collection_Attempts', 'Legal_Action_Taken']],
